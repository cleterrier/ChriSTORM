...
- *Correct astigmatism compression*: Allow to correct for the distortion induced by the astigmatic lens for 3D-STORM. The lense produces a compression along the X axis that is specified to be around 2%, and is compensated at the is value by the N-STORM software. On our N-STORM microscope it was measured at a bit more than that, hence the need for an additional correction factor of 1.01615 (this value is hardcoded in the routine F-NStxtTranslate.js). Uncheck if you don't need to overcorrect astigmatism compression.
