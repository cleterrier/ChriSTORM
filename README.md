...
- Batch process ThunderSTORM localization files: transformation, drift correction, filtering.
- Batch generate image reconstructions from ThunderSTORM localization files (2D, 3D, with various options)
...
- *Correct astigmatism compression*: Allow to overcorrect for the distortion induced by the astigmatic lens for 3D-STORM. The lens produces a compression along the X axis that is specified to be around 2%, and is compensated at this value by the N-STORM software when exproting localizations as text files. On our N-STORM microscope, compression was measured to be a bit more than that (using a field of beads), hence the need for an additional correction factor of 1.01615 (this value is hardcoded in the routine F-NStxtTranslate.js). This allows the 3D-STORM images to be exactly superimposable with their corresponding epifluorescence images. Uncheck if you don't need to overcorrect astigmatism compression.
