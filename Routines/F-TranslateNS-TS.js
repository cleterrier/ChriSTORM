// F-TranslateNS_TS.js script function by Christophe Leterrier
...
function TranslateNSTS(inPath, outDir, outFormat, UseXYDriftCor, UseWarped, UseZDriftCor, factZ, XCorr){
	...
	2D case:
	frame = Frame [12]
	x [nm] = X [1] or Xc [3] or Xw [20] or Xwc [22]
	y [nm] = Y [2] or Yc [4] or Yw [21] or Ywc [23]
	sigma [nm] calculated from Width [7]
	intensity [photon] = Photons [18]
	uncertainty_xy[nm] = Lateral Localization Accuracy [19]
	detections = length [13]

	3D case, add:
	sigma1 [nm] and sigma2 [nm] calculated from Width [7] and Ax [8]
	z [nm] = Z [16] or Zc [17] or Zw [24] or Zwc [25]
	uncertainty_z [nm] = calculated from Lateral Localization Accuracy [19]
	*/
	...
	var fIndex = arrayFind(inHeaderArray, inHeaderList[12]);
	...
	var sigIndex = arrayFind(inHeaderArray, inHeaderList[7]);
	var intIndex = arrayFind(inHeaderArray, inHeaderList[18]);
	var unxyIndex = arrayFind(inHeaderArray, inHeaderList[19]);
	...
	if (zAccu == 0) {
		...
		var aIndex = arrayFind(inHeaderArray, inHeaderList[8]);
	}
